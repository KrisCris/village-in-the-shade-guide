...
export default function CropPlanner({entities}: {entities:Entity[]}) {
  ...
  const seasonal=entities.filter(e=>e.kind==='crops' && (e.seasons as string[]|undefined)?.includes(season));
  const availability=new Map(seasonal.map(crop=>[crop.id,seedAvailability(crop,entities,year,season)]));
  const candidates=seasonal.filter(crop=>ownedSeeds||availability.get(crop.id)?.available);
  const rows=candidates.flatMap(crop=>cropProcessingOptions(crop,catalog,quality)).filter(row=>!query || normalizeSearch(row.crop.searchText+' '+row.output.searchText).includes(normalizeSearch(query))).sort((a,b)=>{
  ...
  return <>
    <div className="planner-controls"><label>年份<select value={year} onChange={e=>setYear(Number(e.target.value))}><option value={1}>第一年</option><option value={2}>第二年</option><option value={3}>第三年及以后</option></select></label><label>种子来源<select value={ownedSeeds?'owned':'shop'} onChange={e=>setOwnedSeeds(e.target.value==='owned')}><option value="shop">按本年商店开放范围</option><option value="owned">已有种子，不限制商店</option></select></label></div>
    <div className="planner-controls"><label>种植季节<select value={season} onChange={e=>setSeason(e.target.value)}>{Object.entries(seasons).map(([id,name])=><option key={id} value={id}>{name}</option>)}</select></label><label>搜索<input value={query} onChange={e=>setQuery(e.target.value)} placeholder="作物、产物或拼音" /></label><label>排序<select value={sort} onChange={e=>setSort(e.target.value)}><option value="monthNet">当月现金净收入</option><option value="net">整批最终净收益</option><option value="processingGain">加工增值</option><option value="machineDays">机器占用时间</option><option value="name">作物名称</option></select></label><label>方向<select value={direction} onChange={e=>setDirection(e.target.value)}><option value="desc">降序</option><option value="asc">升序</option></select></label></div>
    ...
    <div className="planner-table"><table><thead><tr>{['作物 → 产物','收获次数 / 总量','种子总成本','加工增值','整批最终净收益','当月现金净收入','机器占用天数','全部售完需时'].map(t=><th key={t}>{t}</th>)}</tr></thead><tbody>{rows.map(row=><tr key={row.crop.id+row.process.id} tabIndex={0} onClick={event=>{if (!(event.target as HTMLElement).closest('button')) setSelected(row.crop);}} onKeyDown={event=>{if(event.target===event.currentTarget && ['Enter',' '].includes(event.key)){event.preventDefault();setSelected(row.crop);}}}><td><button onClick={()=>setSelected(row.crop)}>{row.crop.name.zh_hans}</button> → <button onClick={()=>setSelected(row.output)}>{row.output.name.zh_hans}</button></td><td>{row.harvestCount} 次 / {row.harvested} 个</td><td>{fmt(row.seedCost)}</td><td>{fmt(row.processingGain)}</td><td>{fmt(row.net)}</td><td>{fmt(row.monthNet)}</td><td>{fmt(row.machineDays)}</td><td>{fmt(row.finishDay)} 天{row.finishDay>28?' · 跨月':''}</td></tr>)}</tbody></table></div>
