...
export default function CropPlanner({entities}: {entities:Entity[]}) {
  ...
  const rows=candidates.flatMap(crop=>cropProcessingOptions(crop,catalog,quality,28,true)).filter(row=>!query || normalizeSearch(row.crop.searchText+' '+row.output.searchText).includes(normalizeSearch(query))).sort((a,b)=>{
