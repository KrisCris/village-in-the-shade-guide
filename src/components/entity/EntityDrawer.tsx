...
export default function EntityDrawer({ initial, onClose }: { initial: Entity; onClose: () => void }) {
  ...
  useEffect(() => {
    fetch('/game-data.json').then((r) => r.json()).then((data) => {
  ...
  return <div className="drawer-backdrop" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
    <aside className="drawer" role="dialog" aria-modal="true" aria-label={title}>
      ...
      <div className="drawer-body">{catalog ? <EntityDetail entity={current} catalog={catalog} onOpen={(next) => setStack([...stack, next])} /> : <p>正在读取关联数据…</p>}</div>
    </aside>
    <style>{`
