...
export default function EntityPage({ initialEntity, initialModel }: { initialEntity: Entity; initialModel: EntityDetailModel }) {
  ...
  const model = useMemo(() => catalog ? buildEntityDetailModel(catalog.byId[initialEntity.id] ?? initialEntity, catalog, quality) : initialModel, [catalog, initialEntity, initialModel, quality]);
  return <>
